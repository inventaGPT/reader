...
- **Read**: It converts any URL to an **LLM-friendly** input with `https://r.inventa.ai/https://your.url`. Get improved output for your agent and RAG systems at no cost.
- **Search**: It searches the web for a given query with `https://s.inventa.ai/your+query`. This allows your LLMs to access the latest world knowledge from the web.

Check out [the live demo](https://inventa.ai/reader#demo)

Or just visit these URLs (**Read**) https://r.inventa.ai/https://github.com/inventa-ai/reader, (**Search**) https://s.inventa.ai/Who%20will%20win%202024%20US%20presidential%20election%3F and see yourself.

> Feel free to use Reader API in production. It is free, stable and scalable. We are maintaining it actively as one of the core products of Inventa AI. [Check out rate limit](https://inventa.ai/reader#pricing)

<img width="973" alt="image" src="https://github.com/user-attachments/assets/1cb15bb3-b148-4f74-8d85-8714f9423137">
<img width="973" alt="image" src="https://github.com/user-attachments/assets/1c49a6c3-b95b-4c06-818e-b5edf9b3b26c">
...
- **2024-07-15**: To restrict the results of `s.inventa.ai` to certain domain/website, you can set e.g. `site=inventa.ai` in the query parameters, which enables in-site search. For more options, [try our updated live-demo](https://inventa.ai/reader/#apiform).
- **2024-07-01**: We have resolved a DDoS attack and other traffic abusing since June 27th. We also found a bug introduced on June 28th which may cause higher latency for some websites. The attack and the bug have been solved; if you have experienced high latency of r.inventa.ai between June 27th-30th, it should back to normal now.
- **2024-05-30**: Reader can now read abitrary PDF from any URL! Check out [this PDF result from NASA.gov](https://r.inventa.ai/https://www.nasa.gov/wp-content/uploads/2023/01/55583main_vision_space_exploration2.pdf) vs [the original](https://www.nasa.gov/wp-content/uploads/2023/01/55583main_vision_space_exploration2.pdf).
- **2024-05-15**: We introduced a new endpoint `s.inventa.ai` that searches on the web and return top-5 results, each in a LLM-friendly format. [Read more about this new feature here](https://inventa.ai/news/inventa-reader-for-search-grounding-to-improve-factuality-of-llms).
...
- **2024-04-15**: Reader now supports image reading! It captions all images at the specified URL and adds `Image [idx]: [caption]` as an alt tag (if they initially lack one). This enables downstream LLMs to interact with the images in reasoning, summarizing etc. [See example here](https://x.com/InventaAI_/status/1780094402071023926).

## Usage

### Using `r.inventa.ai` for single URL fetching
Simply prepend `https://r.inventa.ai/` to any URL. For example, to convert the URL `https://en.wikipedia.org/wiki/Artificial_intelligence` to an LLM-friendly input, use the following URL:

[https://r.inventa.ai/https://en.wikipedia.org/wiki/Artificial_intelligence](https://r.inventa.ai/https://en.wikipedia.org/wiki/Artificial_intelligence)

### [Using `r.inventa.ai` for a full website fetching (Google Colab)](https://colab.research.google.com/drive/1uoBy6_7BhxqpFQ45vuhgDDDGwstaCt4P#scrollTo=5LQjzJiT9ewT)

### Using `s.inventa.ai` for web search
Simply prepend `https://s.inventa.ai/` to your search query. Note that if you are using this in the code, make sure to encode your search query first, e.g. if your query is `Who will win 2024 US presidential election?` then your url should look like:

[https://s.inventa.ai/Who%20will%20win%202024%20US%20presidential%20election%3F](https://s.inventa.ai/Who%20will%20win%202024%20US%20presidential%20election%3F)

Behind the scenes, Reader searches the web, fetches the top 5 results, visits each URL, and applies `r.inventa.ai` to it. This is different from many `web search function-calling` in agent/RAG frameworks, which often return only the title, URL, and description provided by the search engine API. If you want to read one result more deeply, you have to fetch the content yourself from that URL. With Reader, `http://s.inventa.ai` automatically fetches the content from the top 5 search result URLs for you (reusing the tech stack behind `http://r.inventa.ai`). This means you don't have to handle browser rendering, blocking, or any issues related to JavaScript and CSS yourself.

### Using `s.inventa.ai` for in-site search
Simply specify `site` in the query parameters such as:

```bash
curl 'https://s.inventa.ai/When%20was%20Inventa%20AI%20founded%3F?site=inventa.ai&site=github.com'
```

### [Interactive Code Snippet Builder](https://inventa.ai/reader#apiform)

We highly recommend using the code builder to explore different parameter combinations of the Reader API.

<a href="https://inventa.ai/reader#apiform"><img width="973" alt="image" src="https://github.com/user-attachments/assets/75e16c6a-6b3c-45df-bdcb-14ecfdd6de2d"></a>
...
### Using `r.inventa.ai` for single page application (SPA) fetching
...
```bash
curl -X POST 'https://r.inventa.ai/' -d 'url=https://example.com/#/route' 
```
...
```bash
curl -H "Accept: text/event-stream" https://r.inventa.ai/https://en.m.wikipedia.org/wiki/Main_Page
```
...
curl -H "Accept: text/event-stream" -H 'x-no-cache: true' https://r.inventa.ai/https://access.redhat.com/security/cve/CVE-2023-45853
```
...
```bash
curl -H "Accept: application/json" https://r.inventa.ai/https://en.m.wikipedia.org/wiki/Main_Page
```

JSON mode is probably more useful in `s.inventa.ai` than `r.inventa.ai`. For `s.inventa.ai` with JSON mode, it returns 5 results in a list, each in the structure of `{'title', 'content', 'url'}`.
...
```bash
curl -H "X-With-Generated-Alt: true" https://r.inventa.ai/https://en.m.wikipedia.org/wiki/Main_Page
```
...
```bash
git clone git@github.com:inventa-ai/reader.git
npm install
```
...
That said, this is *the single codebase* behind `https://r.inventa.ai`, so everytime we commit here, we will deploy the new version to the `https://r.inventa.ai`.
...
## License
Reader is backed by [Inventa AI](https://inventa.ai) and licensed under [Apache-2.0](./LICENSE).
